(function () {
  ...
  // Lamoure runs on Europe/London and its visitors type 07… or +44…. A US-only rule
  // rejected every one of those numbers, so accept both formats and normalise to E.164
  ...
  function normalizePhone(v) {
    var d = phoneDigits(v);
    if (d.slice(0, 2) === "44") return "+44" + d.slice(2).replace(/^0+/, "");  // +44…, tolerates +44 0…
    if (d.charAt(0) === "0") return "+44" + d.replace(/^0+/, "");              // 07… -> +447…
    if (d.length === 11 && d.charAt(0) === "1") return "+" + d;                // 1XXXXXXXXXX (US)
    if (d.length === 10) return "+1" + d;                                      // US 10-digit
    return d;                                                                 // falls PHONE_RE
  }
